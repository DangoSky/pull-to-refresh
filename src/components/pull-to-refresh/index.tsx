...
class PullRefresh extends PureComponent<PullRefreshProps> {
  ...
  renderHeader = () => {
    ...
    // TODO: icon
    const headerText = headerStatus === STATUS.pullToRefresh ? (
      <>
    ...
    return headerStatus === STATUS.init ? null : headerText;
  }
  ...
  scroll = (e: any) => {
    ...
    const { hasMore, refreshCallback, distancePullUpLoad = 0 } = this.props;
    const { footerStatus } = this.state;
    if (
      ref &&
      hasMore &&
